        ...
        .then((shelf) => {
            ...
            .then((foundbook) => {
                if (foundbook == undefined) {
                    Books.add(book)
                    .then((bk) => {
                        const newUserBookObject = helper.createUserBook(bk, userId, favorite, status, userRating);
                    ...
                    UserBooks.isBookInUserBooks(userId, foundbook.googleId)
                        ...
                        .then((inlibrary) => {
                            ...
                            const userBookObject = helper.createUserBook(foundbook, userId, favorite, status, userRating);
...
router.delete("/shelves/:shelfId/:bookId", (req, res) => {
    ...
    } else { res.status(400).json({ message: "Could not delete book on shelf" })}
});
    ...
    if ((bookId, shelfId, newShelfId)) {
        BooksOnShelf.update(bookId, shelfId, newShelfId)
            .then((updated) => {
                ...
                } else { res.status(500).json({ message: "check bookId, shelfId and newShelfId" })}
            })
            .catch(({ name, message, stack }) => res.status(500).json({ error: "error in moving book to shelf", name, message, stack}));
    } else { res.status(400).json({ message: "Could not update book on shelf, This endpoint requires bookId, shelfId and newShelfId"})}
});
...
router.get("/shelves/:shelfId", (req, res) => {
    ...
    } else { res.status(404).json({ message: "no shelf id exist" })}
});
