...
router.get("/:userId/library/:bookId", (req, res) => {
    ...
    UserBooks.findDetailByUserId(userId, bookId)
        .then(userbook => {
            ...
            } else { res.status(200).json(userbook)}
        })
    ...
    UserBooks.findDetailByUserId(userId, bookId).then(userBook => {
        ...
        UserBooks.update(userId, bookId, 
            ...
            .then(updated => {
                ...
                } else { res.status(201).json({ message: "user book updated successfully" })}
            })
                ...
                if (deleted == 0) {
                    ...
                    BooksOnAShelf.removeAll(bookId, userId)
                        .then(removed => {
                            ...
                            } else { res.status(200).json({message: "book deleted from user library"})}
                        })
...
router.post("/:userId/library", (req, res) => {
    ...
    if (book) {
        ...
        UserBooks.isBookInUserBooks(userId, googleId).then(library => {
            ...
            } else { res.status(400).json({ message: "Book already exists in user library" }) }
        })
    ...
    } else { res.status(400).json({ message: "Please provide a book" })}
});
