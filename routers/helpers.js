module.exports = {
  ...
  findIn: function (req, res, Action, param, where) {
    Action(param)
      ...
      .catch(({ name, message, stack }) =>
        res
          .status(500)
          .json({ error: "error in returning data", name, message, stack })
      );
  },
  ...
  addToUserBooks: async function (req, res, Model, userbookObject) {
    await Model.add(userbookObject)
      .then((added) =>
        res
          .status(201)
          .json({ message: "book added successfully to user library", added })
      )
      .catch(({ name, message, stack }) =>
        res
          .status(500)
          .json({ error: "Error in posting userbook", name, message, stack })
      );
  },
  ...
  addToUserShelf: function (req, res, Model, shelfId, bkId) {
    Model.findBook(shelfId, bkId)
          ...
          if ((bkId, shelfId)) {
            ...
            Model.addBooks(bookObj)
              .then((book) =>
                res.status(201).json({
                  book,
                  message: "book successfully added to user shelf",
                })
              )
              .catch(({ name, message, stack }) =>
                res.status(400).json({
                  error:
                    "error in adding book to shelf. Book may already exist on shelf.",
                  name,
                  message,
                  stack,
                })
      ...
      .catch(({ name, message, stack }) =>
        res.status(500).json({ error: "error occurred", name, message, stack })
      );
  },
};
