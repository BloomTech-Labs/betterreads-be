module.exports = {
    ...
    addToUserBooks: async function (req, res, Model, userbookObject) {
        await Model.add(userbookObject)
            .then((added) =>
                res.status(201).json({ message: "book added successfully to user library", added })
            )
            .catch(({ name, message, stack }) =>
                res.status(500).json({ error: "Error in posting userbook", name, message, stack })
            );
    },
                ...
                if (bookonshelf.length > 0) {
                    ...
                    if ((bkId, shelfId)) {
                        ...
                        Model.addBooks(bookObj)
                            ...
                            .catch(({ name, message, stack }) => res.status(400).json({ error: "error in adding book to shelf. Book may already exist on shelf.", name, message, stack }));
                } else {
